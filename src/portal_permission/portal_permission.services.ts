...
export class PortalUserPermissionService {
  ...
  async get_user_permissions(id_user: number, id_portal_app: string) {
    try {
      ...
      const permissionString = this.aesEcb.encryptToBase64Url(
  ...
  async saveUserPermission(data: CreateUserPermissionDTO) {
    ...
    const decUserId = Number(this.aesEcb.decryptBase64Url(String(id_user)));
    ...
    const decCreateBy = Number(this.aesEcb.decryptBase64Url(String(create_by)));
